...
export default function StudentsPage() {
  ...
  return <div className="page-container bg-slate-50"><Header title={memberLabel} showBack={false} showPoints={false}/><main className="content-area space-y-4 px-4 py-4">
    <section className="rounded-3xl bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{[['missing',`미제출 ${memberLabel}`],['pending','승인 대기'],['feedback','피드백'],['recent','최근 활동']] .map(([key,label]) => <button key={key} onClick={() => update('status', key)} className="min-h-11 shrink-0 rounded-full bg-purple-50 px-3 text-xs font-black text-purple-700">{label}</button>)}</div>
