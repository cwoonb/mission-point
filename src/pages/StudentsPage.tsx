...
export default function StudentsPage() {
  ...
  return <div className="page-container bg-slate-50"><Header title="학생" showBack={false} showPoints={false}/><main className="content-area space-y-4 px-4 py-4">
    <section className="rounded-3xl bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between"><div><h2 className="font-black text-slate-800">최근 활동 조회</h2><p className="text-xs text-slate-400">기본 최근 30일 · 조건은 새로고침 후 유지</p></div><button onClick={() => setParams({})} className="min-h-11 px-2 text-xs font-black text-purple-600">초기화</button></div>
      ...
      <div className="relative mt-2"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(e) => update('q', e.target.value || undefined)} placeholder="학생 이름 검색" className="min-h-11 w-full rounded-xl bg-slate-50 pl-9 pr-3 text-sm outline-none"/></div>
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{[['missing','미제출 학생'],['pending','승인 대기'],['feedback','피드백'],['recent','최근 활동']] .map(([key,label]) => <button key={key} onClick={() => update('status', key)} className="min-h-11 shrink-0 rounded-full bg-purple-50 px-3 text-xs font-black text-purple-700">{label}</button>)}</div>
      {applied > 0 && <span className="mt-2 inline-block rounded-full bg-purple-600 px-2 py-1 text-[10px] font-black text-white">필터 {applied}</span>}
    </section>
    <section className="space-y-2">{rows.map((row) => <button key={row.user.id} onClick={() => navigate(`/students/${row.user.id}?period=${period}`)} className="w-full rounded-2xl bg-white p-3 text-left shadow-sm"><div className="flex items-center gap-3"><span className="text-2xl">{row.user.avatar}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-900">{row.user.name}</strong><span className="text-[11px] font-bold text-slate-400">{row.className} · {row.last ? `${new Date(row.last.createdAt).toLocaleDateString('ko-KR')} 최근 활동` : '활동 기록 없음'}</span></span></div>{row.stats.total === 0 ? <p className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-400">선택한 기간에 배정된 미션 없음</p> : <div className="mt-2 grid grid-cols-4 gap-1 text-center text-[9px] font-black"><span className="rounded-lg bg-emerald-50 py-1 text-emerald-600">완료 {row.stats.completed}</span><span className="rounded-lg bg-blue-50 py-1 text-blue-600">진행 {row.stats.inProgress}</span><span className="rounded-lg bg-red-50 py-1 text-red-600">미제출 {row.stats.missing}</span><span className="rounded-lg bg-orange-50 py-1 text-orange-600">승인 {row.stats.pending}</span></div>}</button>)}{rows.length === 0 && <div className="rounded-3xl bg-white py-12 text-center"><p className="font-bold text-slate-600">선택한 기간에 기록된 활동이 없습니다.</p><button onClick={() => update('period','30')} className="mt-3 min-h-11 rounded-xl bg-purple-600 px-4 text-sm font-black text-white">최근 30일 보기</button></div>}</section>
  </main></div>;
}
