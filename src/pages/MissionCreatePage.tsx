...
import { BookOpen, Check, UserRound, Users } from 'lucide-react';
...
export default function MissionCreatePage() {
  ...
  const selfTask = targetMode === 'self';
  const targetIds = selfTask && currentUser ? [currentUser.id] : targetMode === 'group' ? students.filter((student) => student.groupId === groupId).map((student) => student.id) : selectedIds;
  const valid = !!currentUser && !!title.trim() && (selfTask || !!description.trim()) && targetIds.length > 0 && !!startDate && !!endDate && startDate <= endDate;

  const save = async () => {
    if (!valid || !currentUser || saving) { setError(selfTask ? '할 일 제목과 마감일을 확인해 주세요.' : '제목, 설명, 대상, 기간을 모두 확인해 주세요.'); return; }
    setSaving(true); setError('');
    const base = { title: title.trim(), description: description.trim() || title.trim(), rewardPoint: 0, creatorId: currentUser.id, submissionType: selfTask ? 'TEXT' as const : submissionType, startDate: new Date(`${startDate}T00:00:00`).toISOString(), endDate: new Date(`${endDate}T23:59:59`).toISOString(), missionType: selfTask ? 'OTHER' as const : missionType, missionGoal: 'STUDY_HABIT' as const, repeatType, parentShare: selfTask ? 'NONE' as const : parentShare };
    try {
      await Promise.all(targetIds.map((assigneeId) => createMission({ ...base, assigneeId })));
      navigate(selfTask ? '/missions?tab=self' : '/missions', { replace: true });
    } catch (cause) {
  ...
  return (
    <div className="page-container bg-[#F8F5F0]">
      <Header title={selfTask ? '내 할 일 작성' : '새 미션 만들기'} showBack showPoints={false} rightElement={<button type="button" onClick={save} disabled={!valid || saving} className="min-h-11 px-2 text-sm font-bold text-[#14233B] disabled:text-[#A7ABB2]">{saving ? '저장 중' : '저장'}</button>}/>
      <main className="content-area space-y-6 px-4 py-5">
        {!selfTask && <button type="button" onClick={() => setTemplateOpen(true)} className="flex min-h-12 w-full items-center gap-3 rounded-[10px] border border-[#D8D0C5] bg-[#FFFDFC] px-3 text-left"><BookOpen size={17} className="text-[#B58A4A]"/><span className="flex-1 text-sm font-semibold text-[#14233B]">템플릿에서 불러오기</span><span className="text-xs text-[#8B929C]">{templates.length}개</span></button>}

        <section>
          <label htmlFor="mission-title" className="text-xs font-bold text-[#53606F]">{selfTask ? '내 할 일' : '미션 제목'}</label>
          <input id="mission-title" value={title} onChange={(event) => setTitle(event.target.value)} maxLength={50} placeholder={selfTask ? '예: 김민준 학생 부모님께 리포트 보내기' : '미션 제목을 입력하세요'} className={inputClass}/>
        </section>
        <section>
          <label htmlFor="mission-description" className="text-xs font-bold text-[#53606F]">{selfTask ? '메모 · 선택' : '설명'}</label>
          <textarea id="mission-description" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={300} rows={selfTask ? 3 : 4} placeholder={selfTask ? '확인할 학생이나 준비할 내용을 적어 두세요.' : '학생이 해야 할 내용을 구체적으로 적어주세요.'} className={`${inputClass} resize-none py-3`}/>
        </section>

        <section>
          <h2 className="text-xs font-bold text-[#53606F]">대상</h2>
          <div className="mt-2 grid grid-cols-3 rounded-[10px] bg-[#EDE9E3] p-1">{([['self', '나에게'], ['group', '반 선택'], ['student', '학생 선택']] as const).map(([key, label]) => <button type="button" key={key} onClick={() => setTargetMode(key)} className={`min-h-10 rounded-[8px] px-1 text-[11px] font-bold ${targetMode === key ? 'bg-[#FFFDFC] text-[#14233B] shadow-sm' : 'text-[#7D8490]'}`}>{label}</button>)}</div>
          {targetMode === 'self' ? <div className="mt-2 flex min-h-14 items-center gap-3 rounded-[10px] border border-[#D8D0C5] bg-[#FFFDFC] px-4"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#E9EDF2] text-[#14233B]"><UserRound size={18}/></span><span><strong className="block text-sm text-[#27313F]">{currentUser?.name ?? '나'}</strong><span className="text-[10px] text-[#8B929C]">운영 업무로 내 목록에 추가됩니다.</span></span></div> : targetMode === 'group' ? <select value={groupId} onChange={(event) => setGroupId(event.target.value)} className={inputClass}><option value="">반을 선택하세요</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name} · {students.filter((student) => student.groupId === group.id).length}명</option>)}</select> : <div className="mt-2 max-h-64 divide-y divide-[#ECE7E0] overflow-y-auto rounded-[10px] border border-[#D8D0C5] bg-[#FFFDFC]">{students.map((student) => { const checked = selectedIds.includes(student.id); return <button type="button" key={student.id} onClick={() => toggleStudent(student.id)} className="flex min-h-12 w-full items-center gap-3 px-3 text-left"><span className={`flex h-5 w-5 items-center justify-center rounded border ${checked ? 'border-[#14233B] bg-[#14233B] text-white' : 'border-[#C8C1B8]'}`}>{checked && <Check size={13}/>}</span><span className="flex-1 text-sm font-semibold text-[#27313F]">{student.name}</span><span className="text-[10px] text-[#8B929C]">{groups.find((group) => group.id === student.groupId)?.name ?? '미배정'}</span></button>; })}{students.length === 0 && <EmptyState title="등록된 학생이 없습니다."/>}</div>}
          {targetIds.length > 0 && !selfTask && <p className="mt-2 flex items-center gap-1.5 text-xs font-bold text-[#4F8A68]"><Users size={14}/>{targetIds.length}명에게 배정됩니다.</p>}
        </section>
        ...
        <section>
          <h2 className="text-xs font-bold text-[#53606F]">{selfTask ? '마감일' : '기간 설정'}</h2>
          {selfTask ? <input aria-label="마감일" type="date" min={startDate} value={endDate} onChange={(event) => setEndDate(event.target.value)} className={inputClass}/> : <div className="mt-2 grid grid-cols-2 gap-2"><label className="text-[10px] text-[#8B929C]">시작일<input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} className={`${inputClass} mt-1`}/></label><label className="text-[10px] text-[#8B929C]">종료일<input type="date" min={startDate} value={endDate} onChange={(event) => setEndDate(event.target.value)} className={`${inputClass} mt-1`}/></label></div>}
        </section>

        {!selfTask && <section><label htmlFor="submission-type" className="text-xs font-bold text-[#53606F]">제출 형식</label><select id="submission-type" value={submissionType} onChange={(event) => setSubmissionType(event.target.value as SubmissionType)} className={inputClass}>{SUBMISSIONS.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></section>}
        ...
        {!selfTask && <label className="flex min-h-12 items-center justify-between rounded-[10px] border border-[#D8D0C5] bg-[#FFFDFC] px-3"><span><strong className="block text-sm text-[#27313F]">완료 후 리포트에 포함</strong><span className="text-[10px] text-[#8B929C]">보호자 활동 리포트에서 확인할 수 있습니다.</span></span><input type="checkbox" checked={parentShare !== 'NONE'} onChange={(event) => setParentShare(event.target.checked ? 'ON_COMPLETE' : 'NONE')} className="h-5 w-5 accent-[#14233B]"/></label>}
        {error && <p role="alert" className="rounded-[10px] bg-[#F8EAE8] px-3 py-2 text-xs font-bold text-[#A14E49]">{error}</p>}
        <button type="button" onClick={save} disabled={!valid || saving} className="min-h-12 w-full rounded-[10px] bg-[#14233B] text-sm font-bold text-white disabled:bg-[#A7ABB2]">{saving ? '저장 중...' : selfTask ? '내 할 일 추가' : targetIds.length > 0 ? `${targetIds.length}명에게 미션 저장` : '미션 저장'}</button>
      </main>
