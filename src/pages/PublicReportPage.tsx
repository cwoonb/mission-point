...
export default function PublicReportPage(){
  ...
  useEffect(()=>{if(!secureBackendEnabled){setLoading(false);return;}void (async()=>{const {data,error}=await supabase.rpc('get_public_report',{raw_token:token??''});if(!error&&data)setReport(data as unknown as PublicReport);setLoading(false);})();},[token]);
  ...
  return <div className="page-container bg-[#F8F5F0]"><main className="px-4 py-6"><article className="overflow-hidden rounded-[18px] border border-[#E7E1D9] bg-[#FFFDFC] shadow-[0_8px_28px_rgba(20,35,59,.05)]">
    ...
    <div className="space-y-7 p-5">
      ...
      <section><h2 className="mb-3 flex items-center gap-2 text-sm font-bold text-[#14233B]"><MessageCircle size={17} className="text-[#B58A4A]"/>선생님 피드백</h2>{report.feedback.length?<div className="space-y-2">{report.feedback.map((item,index)=><div key={`${item.date}-${index}`} className="rounded-xl bg-[#F5F2ED] p-3"><strong className="text-xs text-[#14233B]">{item.mission}</strong><p className="mt-2 text-sm leading-6 text-[#53606F]">{item.text}</p></div>)}</div>:<Empty>최근 피드백이 없습니다.</Empty>}</section>
      ...
      <section className="rounded-xl border border-[#D8D0C5] bg-[#F3EFE9] p-4"><h2 className="text-xs font-bold text-[#B58A4A]">선생님 한줄 메모</h2><p className="mt-2 text-sm font-semibold leading-6 text-[#27313F]">{report.teacherMemo}</p></section>
