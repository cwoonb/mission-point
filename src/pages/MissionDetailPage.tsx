...
export default function MissionDetailPage() {
  ...
  const selfTask = active?.role !== 'STUDENT' && mission.creatorId === currentUser.id && mission.assigneeId === currentUser.id;
  if (selfTask) {
    const completed = mission.status === 'SUCCESS';
    return <div className="page-container bg-[#F8F5F0]"><Header title="내 할 일 상세" showBack showPoints={false}/><main className="content-area space-y-4 px-4 py-4">
      ...
      <button type="button" onClick={() => { updateStatus(mission.id, completed ? 'IN_PROGRESS' : 'SUCCESS'); navigate('/missions?tab=self', { replace: true }); }} className={`flex min-h-12 w-full items-center justify-center gap-2 rounded-[10px] text-sm font-bold ${completed ? 'border border-[#D8D0C5] bg-[#FFFDFC] text-[#53606F]' : 'bg-[#14233B] text-white'}`}>{completed ? <><Circle size={17}/>다시 할 일로</> : <><CheckCircle2 size={17}/>완료하기</>}</button>
    </main></div>;
  }
  ...
  return <div className="page-container bg-[#F8F5F0]"><Header title="미션 상세" showBack showPoints={false}/><main className="content-area space-y-4 px-4 py-4">
    ...
    {mission.status==='SUCCESS'&&<section className="rounded-[14px] border border-[#CFE0D5] bg-[#F1F7F3] p-4"><h2 className="flex items-center gap-2 text-sm font-bold text-[#315E43]"><CheckCircle2 size={17}/>승인 완료</h2>{approved?.publicFeedback&&<p className="mt-3 text-sm leading-6 text-[#456650]">{approved.publicFeedback}</p>}</section>}
    ...
    {logs.some((log)=>log.reason||log.publicFeedback)&&<section><h2 className="mb-3 flex items-center gap-2 text-sm font-bold text-[#14233B]"><MessageCircle size={17}/>선생님 피드백</h2><div className="space-y-2">{logs.filter((log)=>log.reason||log.publicFeedback).map((log)=><article key={log.id} className="rounded-[12px] border border-[#E7E1D9] bg-[#FFFDFC] p-4"><div className="flex justify-between gap-2"><strong className={`text-xs ${log.action==='REJECTED'?'text-[#A65F59]':'text-[#4F8A68]'}`}>{log.action==='REJECTED'?'수정 요청':'승인'}</strong><time className="text-[10px] text-[#9A9FA7]">{formatDateTime(log.createdAt)}</time></div><p className="mt-2 text-sm leading-6 text-[#53606F]">{log.action==='REJECTED'?log.reason:log.publicFeedback}</p></article>)}</div></section>}
