...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!active) return;
      // Reload on the destination so App performs session/profile restoration
      // exactly once. Updating the auth store while still on this route remounts
      // the callback and can create an endless loading loop after `code` is removed.
      window.location.replace(next);
    })();
    return () => { active = false; };
  }, [code, oauthErrorCode, oauthErrorDescription, provider, requestedNext]);
