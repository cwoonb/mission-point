...
export default function ReportSharePage() {
  ...
  useEffect(() => {
    if (active?.organizationId && id) void load(active.organizationId, id).catch(() => setNotice('저장된 공개 메모를 불러오지 못했습니다.'));
  }, [active?.organizationId, id, load]);
  ...
  }, [active?.organizationId, groups, missions, reviewLogs, student, submissions, teacherMemo]);
  ...
  return <div className="page-container bg-[#F8F5F0]">
    ...
    <main className="content-area flex min-h-[calc(100dvh-9rem)] flex-col items-center justify-center px-6 py-10 text-center">
      ...
      <p className="mt-3 rounded-xl bg-[#F3EFE9] px-4 py-3 text-xs leading-5 text-[#687282]">공개 링크는 생성 시점의 내용으로 고정됩니다. 피드백이나 메모를 수정했다면 새 링크를 만들어 주세요.</p>
