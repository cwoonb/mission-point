...
import { RARITY_LABEL, SPECIES_LABEL, MAX_STAGE } from '../config/pets';
        ...
        {!isFacilitator && (
          <motion.button
            ...
            <div className="flex-1 text-left text-white">
              {activePet ? (
                <>
                  <p className="font-black text-lg">{activePet.name || SPECIES_LABEL[activePet.species]}</p>
                  <p className="text-xs text-white/80 mb-1">{RARITY_LABEL[activePet.rarity]} · {activePet.stageIndex}/{MAX_STAGE[activePet.rarity]}단계</p>
                </>
              ) : (
                <p className="font-black text-lg mb-1">마이 펫</p>
              )}
              <p className="text-[11px] bg-white/20 inline-block rounded-full px-2.5 py-1 font-semibold">🐾 마이 펫 보기 →</p>
            </div>
