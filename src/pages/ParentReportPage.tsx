...
export default function ParentReportPage() {
  ...
  return <div className="min-h-screen bg-[#F3EFE9] py-4">
    <main className="mx-auto w-full max-w-md px-4 pb-24">
      <div className="mb-4 flex items-center gap-3"><button onClick={() => navigate(-1)} aria-label="뒤로 가기" className="flex h-11 w-11 items-center justify-center rounded-[10px] border border-[#E7E1D9] bg-[#FFFDFC] text-[#687282]"><ArrowLeft size={18}/></button><div className="min-w-0 flex-1"><h1 className="font-bold text-[#14233B]">보호자 활동 리포트</h1><p className="text-[11px] font-medium text-[#687282]">{memberLabel}의 최근 활동을 편하게 읽는 기록</p></div><button onClick={handleShare} className="flex min-h-11 items-center gap-1.5 rounded-[10px] bg-[#14233B] px-3 text-xs font-bold text-white"><Share2 size={14}/>공유</button></div>
      ...
      <article className="overflow-hidden rounded-[20px] border border-[#E7E1D9] bg-[#FFFDFC] shadow-[0_8px_28px_rgba(20,35,59,.06)]">
        <header className="border-b border-[#E7E1D9] bg-[#F8F5F0] p-6"><p className="text-xs font-semibold text-[#B58A4A]">{formatDate(new Date(Date.now() - 29 * 86400000).toISOString())} ~ {formatDate(new Date().toISOString())}</p><div className="mt-4 flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-[#E9EDF2] text-xl font-bold text-[#14233B]">{student.profileImage ? <img src={student.profileImage} alt={`${student.name} 프로필`} className="h-full w-full object-cover"/> : student.name.slice(0, 1)}</div><div><h2 className="text-2xl font-bold text-[#14233B]">{student.name}</h2><p className="mt-1 text-xs font-medium text-[#687282]">{groupName} · {teacherName} {facilitatorLabel}</p></div></div></header>
        ...
        <div className="space-y-7 p-5">
          ...
          <section className="rounded-xl border border-[#CFE0D5] bg-[#F1F7F3] p-4"><div className="flex items-center gap-2"><Sparkles size={17} className="text-[#4F8A68]"/><h3 className="font-bold text-[#315E43]">잘한 점</h3></div><p className="mt-2 text-sm leading-relaxed text-[#315E43]">{strengths}</p></section>
          <section className="rounded-xl border border-[#D6DEE8] bg-[#F2F5F8] p-4"><div className="flex items-center gap-2"><Target size={17} className="text-[#5D718F]"/><h3 className="font-bold text-[#40536F]">다음 목표</h3></div><p className="mt-2 text-sm leading-relaxed text-[#40536F]">{nextGoal}</p></section>
          <section className="rounded-xl border border-[#D8D0C5] bg-[#F3EFE9] p-4"><p className="text-xs font-bold text-[#B58A4A]">{facilitatorLabel} 한줄메모</p><p className="mt-2 text-base font-semibold leading-relaxed text-[#27313F]">“{oneLineNote}”</p></section>
        </div>
      </article>
    </main>
  </div>;
