...
export default function ParentReportPage() {
  ...
  if (!student) return <div className="page-container"><main className="content-area flex items-center justify-center text-sm font-bold text-slate-400">학생을 찾을 수 없습니다.</main></div>;

  const shareText = `${student.name} 학생 활동 리포트\n\n최근 활동: ${recentActivity.length ? recentActivity.map((item) => titleFor(item.missionId)).join(', ') : '기록 없음'}\n최근 완료: ${completed.length ? completed.map((mission) => mission.title).join(', ') : '기록 없음'}\n현재 진행: ${active.length ? active.map((mission) => mission.title).join(', ') : '없음'}\n\n잘한 점: ${strengths}\n다음 목표: ${nextGoal}\n선생님 메모: ${oneLineNote}`;
  const handleShare = async () => {
    setShareNotice('');
    try {
      if (typeof navigator.share === 'function') {
        await navigator.share({ title: `${student.name} 학생 활동 리포트`, text: shareText, url: window.location.href });
  ...
  return <div className="min-h-screen bg-slate-100 py-4">
    <main className="mx-auto w-full max-w-md px-4 pb-24">
      <div className="mb-4 flex items-center gap-3"><button onClick={() => navigate(-1)} aria-label="뒤로 가기" className="flex h-11 w-11 items-center justify-center rounded-xl bg-white text-slate-600 shadow-sm"><ArrowLeft size={18}/></button><div className="min-w-0 flex-1"><h1 className="font-black text-slate-900">학부모 활동 리포트</h1><p className="text-[11px] font-bold text-slate-400">학생의 최근 활동을 편하게 읽는 기록</p></div><button onClick={handleShare} className="flex min-h-11 items-center gap-1.5 rounded-xl bg-emerald-600 px-3 text-xs font-black text-white"><Share2 size={14}/>공유</button></div>
      ...
      <article className="overflow-hidden rounded-[28px] bg-white shadow-lg">
        ...
        <div className="space-y-7 p-5">
          ...
          <section><div className="mb-3 flex items-center gap-2"><MessageCircle size={17} className="text-indigo-600"/><h3 className="font-black text-slate-800">선생님 피드백</h3></div>{feedback.length === 0 ? <Empty>최근 전달된 피드백이 없습니다.</Empty> : <div className="space-y-2">{feedback.map((log) => <div key={log.id} className="rounded-2xl bg-indigo-50 p-3"><strong className="text-xs text-indigo-800">{titleFor(log.missionId)}</strong><p className="mt-1 text-sm leading-relaxed text-slate-700">{log.reason}</p></div>)}</div>}</section>
          <section><div className="mb-3 flex items-center gap-2"><Send size={17} className="text-amber-600"/><h3 className="font-black text-slate-800">학생이 수정한 내용</h3></div>{revisions.length === 0 ? <Empty>다시 제출한 기록이 없습니다.</Empty> : <div className="space-y-2">{revisions.map((submission) => <div key={submission.id} className="rounded-2xl bg-amber-50 p-3"><strong className="text-xs text-amber-800">{titleFor(submission.missionId)} · {submission.attemptNumber}회차</strong><p className="mt-1 text-sm leading-relaxed text-slate-700">{submission.message || '내용을 보완해 다시 제출했습니다.'}</p></div>)}</div>}</section>
          ...
          <section className="rounded-3xl border border-violet-100 bg-violet-50 p-4"><p className="text-xs font-black text-violet-600">선생님 한줄메모</p><p className="mt-2 text-base font-bold leading-relaxed text-slate-800">“{oneLineNote}”</p></section>
        </div>
      </article>
    </main>
  </div>;
