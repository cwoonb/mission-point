...
import { CheckCircle2, ChevronRight, MessageCircle, Share2, Target } from 'lucide-react';
...
export default function ParentReportPage() {
  ...
  const feedbackNotes = (teacherNotes[id ?? ''] ?? []).filter((note) => studentMissions.some((mission) => note.text.startsWith(`${mission.title}:`))).slice(0, 3);
  const reviewFeedback = reviewLogs.filter((log) => ids.has(log.missionId) && log.reason).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 3);
      ...
      <main className="content-area px-4 py-4">
        <article className="overflow-hidden rounded-[18px] border border-[#E7E1D9] bg-[#FFFDFC] shadow-[0_8px_28px_rgba(20,35,59,.05)]">
          ...
          <div className="space-y-7 p-5">
            ...
            <section>
              <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-[#14233B]"><MessageCircle size={17} className="text-[#B58A4A]"/>선생님 피드백</h3>
              {feedbackNotes.length === 0 && reviewFeedback.length === 0 ? <Empty>최근 전달된 피드백이 없습니다.</Empty> : <div className="space-y-2">{feedbackNotes.map((note) => <p key={note.id} className="rounded-xl bg-[#F5F2ED] p-3 text-sm leading-6 text-[#53606F]">{note.text.split(':').slice(1).join(':').trim()}</p>)}{reviewFeedback.map((log) => <p key={log.id} className="rounded-xl bg-[#F8ECEA] p-3 text-sm leading-6 text-[#714B48]">{log.reason}</p>)}</div>}
            </section>
            ...
            <section className="rounded-xl border border-[#D8D0C5] bg-[#F3EFE9] p-4">
              <p className="text-xs font-bold text-[#B58A4A]">선생님 한줄 메모</p>
              <p className="mt-2 text-sm font-semibold leading-6 text-[#27313F]">{recentSubmissions.length > 0 ? '꾸준히 참여하며 자신의 학습 과정을 차근차근 만들어가고 있습니다.' : '새로운 활동을 시작할 수 있도록 함께 응원해 주세요.'}</p>
            </section>
          </div>
        </article>
      </main>
