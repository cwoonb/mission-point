...
export const getBottomNavTabs=(role:MembershipRole|undefined,memberLabel='학생',pendingReviews=0,organizationType?:string):NavTab[]=>organizationType==='PERSONAL'?
  [{to:'/',icon:Home,label:'홈'},{to:'/missions',icon:Target,label:'TODO'},{to:'/profile',icon:User,label:'내 정보'}]:role==='STUDENT'?
...
export default function BottomNav() {
  ...
  const activeOrganization = useMembershipStore((state)=>state.organizations.find((organization)=>organization.id===activeMembership?.organizationId));
  ...
  const tabs=getBottomNavTabs(activeMembership?.role,memberLabel,pendingReviews,activeOrganization?.type);
